...
def update_app(app: Deployment) -> Deployment:
    ...
    match app_info["image_check_method"]:
        case "Docker":
            ...
            current_version = url_prefix + app_info["image_chart_name"] + ':' + app_info["latest_version"]
            target_version = url_prefix + app_info["image_chart_name"] + ':' + app_info["target_version"]
        ...
        case "Helm":
            h = HelmChecker()
            if not h.check_helm(app_info["image_url"], app_info["image_chart_name"], app_info["latest_version"]):
                new_version = h.pull_helm_version(app_info["image_url"], app_info["image_chart_name"])
