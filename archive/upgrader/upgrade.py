...
def upgrade(app: Deployment) -> Deployment:
    output_log(
        f"Upgrade handler for {config.local_repo} | {config.git_repo} | {config.git_pass}",
        "debug",
    )
...
def _update_file(
    local_location: str, change: dict, old_version: str, new_version: str
) -> Changes:
    try:
        ...
        with open(f"{local_location}/{change['file']}", "w") as f:
            yaml.dump_all(
                [data] + data_remain, f, default_flow_style=False, explicit_start=True
            )
        return Changes(**change)
    except Exception as e:
        output_log(
            f"Error in updating file {change['file']} in {local_location} from {old_version} to {new_version}: {e}",
            "error",
        )
        return Changes(**change)
